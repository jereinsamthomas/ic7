import{S as f}from"./store-Cr00HOVh.js";import"https://esm.sh/@supabase/supabase-js@2";document.addEventListener("DOMContentLoaded",async()=>{await f.init();const u=document.getElementById("starfield"),v=window.innerWidth<700?60:130;for(let t=0;t<v;t++){const e=document.createElement("span");e.className="star";const s=1+Math.random()*1.8;e.style.width=s+"px",e.style.height=s+"px",e.style.left=Math.random()*100+"%",e.style.top=Math.random()*100+"%",e.style.setProperty("--s-min",(.1+Math.random()*.25).toFixed(2)),e.style.setProperty("--s-dx",(Math.random()*30-15).toFixed(1)+"px"),e.style.setProperty("--s-dy",(-(20+Math.random()*40)).toFixed(1)+"px");const r=(2+Math.random()*4).toFixed(1),i=(18+Math.random()*22).toFixed(1);e.style.animation=`starTwinkle ${r}s ease-in-out infinite, starDrift ${i}s ease-in-out infinite alternate`,e.style.animationDelay=`${(-Math.random()*10).toFixed(1)}s, ${(-Math.random()*10).toFixed(1)}s`,u.appendChild(e)}function l(t){return String(t||"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}const m="club-world-cup",x=f.get("ic7_major_tours",{}),o=Object.assign({name:"Club World Cup",teams:"8 Teams",desc:"Champions of every other competition settle it here."},x[m]||{});if(document.getElementById("tourName").textContent=o.name,document.getElementById("tourDesc").textContent=o.desc,document.getElementById("tourTeams").textContent=o.teams,document.title=o.name+"  -  IC7 Iconic Sevens",o.tourLogo){const t=document.getElementById("tourLogoWrap"),e=document.getElementById("tourLogoImg");t&&e&&(e.src=o.tourLogo,t.style.display="block")}if(o.champion){const t=document.querySelector(".tourpage-meta-row");if(t){const e=document.createElement("div");e.style.display="flex",e.style.flexDirection="column",e.style.gap="4px";const s=o.champLogo?`<div style="width:24px;height:24px;border-radius:50%;background-image:url('${o.champLogo}');background-size:cover;background-position:center;border:1px solid var(--red);box-shadow: 0 0 10px rgba(225,29,72,0.3);"></div>`:"";e.innerHTML=`<span>Current Champion</span><b style="display:flex;align-items:center;gap:8px;color:#fff;">${s}${l(o.champion)}</b>`,t.appendChild(e)}}if(o.shieldWinner){const t=document.querySelector(".tourpage-meta-row");if(t){const e=document.createElement("div");e.style.display="flex",e.style.flexDirection="column",e.style.gap="4px";const s=o.shieldLogo?`<div style="width:24px;height:24px;border-radius:50%;background-image:url('${o.shieldLogo}');background-size:cover;background-position:center;border:1px solid #f59e0b;box-shadow: 0 0 10px rgba(245,158,11,0.3);"></div>`:'<span style="font-size:18px;line-height:1;">🛡️</span>';e.innerHTML=`<span style="color:var(--ink-faint);">Shield Winner</span><b style="display:flex;align-items:center;gap:8px;color:#f59e0b;">${s}${l(o.shieldWinner)}</b>`,t.appendChild(e)}}["standings","scorers","assists","glove","shield"].forEach(t=>{const e=t+"Img",s=document.getElementById(t+"ImgWrap"),r=document.getElementById(t+"Img"),i=document.getElementById("empty_"+t);o&&o[e]?(s&&r&&(r.src=o[e],s.hidden=!1),i&&(i.hidden=!0)):(s&&(s.hidden=!0),i&&(i.hidden=!1))});function n(t,e,s=0){const r=f.get("ic7_tour_"+m+"_"+t,[]),i=document.getElementById("tb_"+t),d=document.getElementById("empty_"+t);if(i){if(!r.length){d.hidden=!1,i.innerHTML="";return}d.hidden=!0,i.innerHTML=r.map((c,p)=>{const a=e.map((h,w)=>{const y=l(c[h]||"");return w===s&&c.logo?`<td><div style="display:flex;align-items:center;gap:10px;"><img src="${c.logo}" style="width:24px;height:24px;border-radius:50%;object-fit:cover;border:1px solid var(--red);box-shadow:0 0 8px rgba(225,29,72,0.4);flex:none;"><span>${y}</span></div></td>`:"<td>"+y+"</td>"}).join("");return'<tr><td class="rank">'+String(p+1).padStart(2,"0")+"</td>"+a+"</tr>"}).join("")}}n("standings",["team","played","won","points"]),n("scorers",["player","team","goals","matches"]),n("assists",["player","team","assists","matches"]),n("glove",["player","team","cleanSheets","matches"]),n("shield",["player","team","season","matches"]);function g(){const t=f.get("ic7_tour_"+m+"_winners",[]),e=document.getElementById("winnersSection"),s=document.getElementById("winnersList");if(!s||!e)return;e.hidden=!1;const r=t.length?t:[{season:"SEASON 1",player:"IL CAPITANO",logo:""},{season:"SEASON 2",player:"ICONIC SHIELD",logo:""}];s.innerHTML=r.map((i,d)=>{const c=l(i.season||`SEASON ${d+1}`),p=l(i.player||"CHAMPION");let a="";if(i.logo)a=`<div class="evo-tc-trophy-badge"><img class="evo-tc-trophy-img" src="${i.logo}" alt="${p}"></div>`;else if(d===0)a=`<div class="evo-tc-trophy-badge">
          ...
          </div>`;else if(d===1)a=`<div class="evo-tc-trophy-badge">
          ...
          </div>`;else{const h=p.charAt(0).toUpperCase();a=`<div class="evo-tc-trophy-badge"><div class="evo-tc-default-crest"><span>${l(h)}</span></div></div>`}return`<div class="evo-tc-bay reveal in-view">
      ...
      </div>`}).join("")}g(),window.addEventListener("ic7_store_updated",()=>{n("standings",["team","played","won","points"]),n("scorers",["player","team","goals","matches"]),n("assists",["player","team","assists","matches"]),n("glove",["player","team","cleanSheets","matches"]),n("shield",["player","team","season","matches"]),g()})});
